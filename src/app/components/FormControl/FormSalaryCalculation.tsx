...
const FormSalaryCalculation = ({ register, errors, getValues, watch, setValue }) => {
  ...
  const handleBlur = () => {
    ...
    // if (select === 'hourly') {
    //   hourlySalary = Number(getValues('hourlySalary')) * 12
    // }
    if (select === 'daily') {
  ...
  return (
    <>
    <FormRadioButtonStyle defaultValue="monthly" options={salaryTypesOpions} title="薪資狀況(新台幣)" error={errors?.salaryTypes?.message} {...register('salaryTypes', { required: "This is required.", onChange: (e) => {setSelect(e.target.value);} })}/>
    <div className="mt-4">
    <div className="">
      <div className="flex flex-wrap md:flex-nowrap">
        <div className="w-full">
          { select === 'monthly' &&
          <>
            <NumberInput placeholder={'月薪 EX:35000'} {...register('monthlySalary')} onBlur={handleBlur}/>
          ...
          {
            select === 'hourly' && <NumberInput placeholder={'時薪 EX:176'} {...register('hourlySalary')} onBlur={handleBlur}/>
          }
        </div>
        { select !== 'monthly' && 
        <div className="md:shrink grow md:w-full flex">
          <div className="w-full">
            <Select options={monthOptions} {...register('avgWorkingDaysPerMonth')}/>
          </div>
        </div>
        }
      </div>
    </div>
    <FormNumberInput placeholder={'年終 EX:12000'} {...register('yearEndBonus')} onBlur={handleBlur} />
    <FormNumberInput placeholder={'三節'} {...register('holidayBonus')} onBlur={handleBlur}/>
    <FormNumberInput placeholder={'獎金'} {...register('profitSharingBonus')} onBlur={handleBlur}/>
    <FormNumberInput placeholder={'其他'} {...register('otherBonus')} onBlur={handleBlur}/>
